/* Breizh’ Balade V2.5.1 — splash + recherche avancée universelle */
const CACHE_NAME = 'breizh-balade-v2.5.1-public';
const APP_SHELL = [
  './','./index.html','./splash.html','./manifest.webmanifest',
  './css/style.css','./css/map.css','./css/responsive.css',
  './js/app.js','./js/search-accordion.js?v=251','./js/storage.js','./js/geolocation.js?v=244','./js/map.js',
...
self.addEventListener('activate',event=>{
  event.waitUntil(
    caches.keys()
      .then(keys=>Promise.all(keys.filter(key=>key.startsWith('breizh-balade-')&&key!==CACHE_NAME).map(key=>caches.delete(key))))
...
const injectAdvancedSearch = async response => {
  ...
  const injected = html.replace('</body>','<script src="./js/search-accordion.js?v=251"></script></body>');
...
const networkFirst=async request=>{
  try{
    ...
    if(request.mode==='navigate'){
      ...
      return injectAdvancedSearch(fallback);
    }
    throw error;
  }
};
...
const serveSplash=async()=>{
  try{
    const response=await fetch('./splash.html?v=249',{cache:'no-store'});
    if(response&&response.ok){
      const cache=await caches.open(CACHE_NAME);
