/* BREIZH’ BALADE — service-worker.js
   V2.1.6.18 : navigation du partage corrigée sans toucher au tracker ANALYTIX. */
const CACHE_NAME = 'breizh-balade-v2.1.6.18-share-fix';
const APP_SHELL = [
...
self.addEventListener('fetch', event => {
  ...
  if (insideScope && request.mode === 'navigate') {
    ...
    if (!splashDone && !isSplash) {
      event.respondWith(
        fetch('./splash.html?v=21618', {cache:'no-store'})
