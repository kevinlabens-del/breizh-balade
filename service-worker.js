/* Breizh’ Balade V2.4.8 — service worker splash universel */
const CACHE_NAME = 'breizh-balade-v2.4.8-public';
const APP_SHELL = [
  './', './index.html', './splash.html', './manifest.webmanifest',
  './css/style.css', './css/map.css', './css/responsive.css',
  './js/app.js', './js/storage.js', './js/geolocation.js?v=244', './js/map.js',
  ...
  './assets/logo/breizh-pattern.svg', './assets/icons/icon-192.png', './assets/icons/icon-512.png',
  './assets/places/coast.svg', './assets/splash/breizh-balade-splash.jpg'
];
...
const networkFirst = async request => {
  try {
    ...
    if (request.mode === 'navigate') return caches.match('./index.html'); throw error;
  }
};
const hotfixScript = async (originalRequest, target) => {
  try {
    ...
    const cache = await caches.open(CACHE_NAME); cache.put(originalRequest, response.clone()).catch(() => {}); return response;
  } catch (error) {
    const cachedTarget = await caches.match(target); if (cachedTarget) return cachedTarget;
    const cachedOriginal = await caches.match(originalRequest); if (cachedOriginal) return cachedOriginal; throw error;
  }
};
const cacheFirst = async request => {
  ...
  try {
    ...
    if (request.destination === 'image') { const fallback = await caches.match('./assets/places/coast.svg'); if (fallback) return fallback; }
    throw error;
  }
};
const serveSplash = async () => {
  try {
    const response = await fetch('./splash.html?v=248', { cache: 'no-store' });
    if (response && response.ok) { const cache = await caches.open(CACHE_NAME); cache.put('./splash.html', response.clone()).catch(() => {}); return response; }
  } catch (_) {}
