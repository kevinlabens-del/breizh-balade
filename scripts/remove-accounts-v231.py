...
p=Path('js/app.js'); s=p.read_text(encoding='utf-8').replace('V2.3.0','V2.3.1')
s=re.sub(r"\n\s*// Connexion obligatoire avant setView — V2\.3\.1\n\s*const publicViews = \['explore', 'map', 'about', 'login', 'signup'\];\n\s*if \(window\.BreizhAuth.*?\n\s*\}\n",'\n',s,flags=re.S)
s=re.sub(r"\n  const requirePersonalAccount = \(\) => \{.*?\n  \};\n","\n  const requirePersonalAccount = () => true;\n",s,flags=re.S)
s=re.sub(r"\n\s*// Connexion obligatoire avant hashchange — V2\.3\.1\n\s*const publicViews = \['explore', 'map', 'about', 'login', 'signup'\];\n\s*if \(window\.BreizhAuth.*?\n\s*\}\n",'\n',s,flags=re.S)
...
Path('js/storage.js').write_text(r'''const Store = (() => {
  ...
  const migrateOldAccountData = () => {
    ...
    for (let i=0;i<localStorage.length;i++) { const k=localStorage.key(i)||''; if (k.startsWith(prefix+'user:') && k.endsWith(':notes')) Object.assign(notes,readJSON(k,{})); }
...
p=Path('manifest.webmanifest'); p.write_text(p.read_text(encoding='utf-8').replace('V2.3.0','V2.3.1'),encoding='utf-8')
